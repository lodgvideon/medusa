...
func main() {
	...
	node, err := medusa.New(medusa.Config{ID: id, Addr: addr, BindAddr: bindAddr, Seeds: seeds, DataDir: dataDir})
	if err != nil {
	...
	slog.Info("node listening", "id", id, "advertise", addr, "bind", node.Addr(), "http", httpAddr, "seeds", seeds)
